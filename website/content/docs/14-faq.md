...
2. **Did the engine emit a `SyncChangeset`?** Subscribe to the change broadcast: `db.change_rx().recv().await`. If you don't see a notification, the write likely didn't go through an intercepted `WaveSyncDb` statement yet — writes made via `db.inner()` or from a separate process sharing the database file (e.g. iOS background sync) are still captured by SQLite triggers, but the capture rows only get drained into a `SyncChangeset` on the *next* intercepted write or at startup. Issue a follow-up write through `WaveSyncDb`, or restart, to flush it.
...
5. **HMAC failing?** Enable `RUST_LOG=wavesyncdb=debug` and search logs for `reason=hmac_verify_failed` or messages containing "rejecting... with invalid HMAC". Almost always a passphrase typo. WaveSyncDB's events go through `tracing`, but `RUST_LOG` still works the same way it always has (`tracing-subscriber`'s `EnvFilter` parses the same directive syntax `env_logger` does) — and if your app initializes logging with `env_logger`, you'll still see these events with no changes on your end.
