//! Convergence verification + recursive range-based set reconciliation (#82).
//!
//! Two peers hold identical group data iff every shadow-table cell agrees on
//! its CRDT version *and* value. A cell is summarised by [`cell_fp`] — a BLAKE3
...
//!   ([`compute_group_digest`]). Equal digests across two peers ⇒ proven
//!   identical (the version-vector catch-up can't prove this).
...
//!
//! Value bytes come from `get_changes_since` (which reads through SQLite), so
//! two native peers that hold the same data produce identical fingerprints. The
//! primary-key column's clock cell is excluded (the writer records one but the
//! receiver rejects PK-column changes, so it's asymmetric; the PK value is
//! already implicit in every cell's key).
...
/// ~`log16(n)` rounds.
pub(super) const RBSR_BUCKET: usize = 16;
...
fn sort_key(table: &str, pk: &str, cid: &str) -> Vec<u8> {
...
pub(super) fn cell_fp(c: &ColumnChange) -> [u8; 32] {
...
fn range_fp(cells: &[LocalCell]) -> [u8; 32] {
    let mut acc = [0u8; 32];
    for c in cells {
...
pub(super) async fn enumerate_sorted_cells(
    ...
    let mut cells: Vec<LocalCell> = changes
        ...
        .filter(|c| {
            registry
                .get(&c.table.0)
                .is_none_or(|m| m.primary_key_column != c.cid.0)
        })
        .map(|c| LocalCell {
...
pub(super) fn initial_entries(local: &[LocalCell]) -> Vec<RangeEntry> {
    if local.is_empty() {
