...
//!
//! Node ID seeds: 80–89 (registered in the project's seed registry); 74–76
//! for the two-group isolation scenario (one native node's default-group
//! identity, plus two web-store site ids — the joined group's own site_id is
//! always freshly generated by `join_group`, never seed-controlled, so no
//! seed is needed for it). `make_node_id` takes a `u8`, so a first-pass idea
//! of reusing a 260–263 range didn't fit the type; 74–76 is the nearest
//! verified-free gap (checked against every `make_node_id(..)` call across
//! `tests/`, not just the documented ranges — a couple of small values
//! inside the registry's documented "1–10" range for `integration.rs` turned
//! out to already be in use undocumented, so grep ground-truth won out over
//! the table here — the registry is a lower bound, not a complete list).
...
// ---------------------------------------------------------------------------
// Multi-group isolation (#93): two web `MemoryStore`s configured for the SAME
// table name (`tasks`), each paired with a DIFFERENT native group DB (one
// libp2p node, two `join_group`-ed groups — mirrors
// `integration_multigroup.rs`'s native-only two-group setup). Isolation here
// is not a scope/descriptor rule — both group DBs and both web configs use
// the identical bare table name — it is a property of which STORE/CONFIG
// PAIRING each exchange happens between. This pins down that no accidental
// process-wide state (a shared registry, a table-name-keyed cache, etc.)
// leaks group A's rows into group B's web store, or vice versa, just because
// they share a table name.
// ---------------------------------------------------------------------------
#[tokio::test]
async fn two_groups_same_table_stay_isolated() {
    common::init_test_tracing();
    let suffix = Uuid::new_v4().simple().to_string();
    let topic_alpha = format!("iso-alpha-{suffix}");
    let topic_beta = format!("iso-beta-{suffix}");

    // One physical node (one swarm/site identity), two joined groups. Each
    // group gets its OWN SQLite file and its OWN `TableRegistry` (see
    // `join_group_inner` / `derive_group_database_url`), even though both
    // register the exact same `tasks` entity/table name.
    let node_alpha = WaveSyncDbBuilder::new(&mem_db("iso_alpha"), &topic_alpha)
        .with_node_id(make_node_id(74))
        .with_passphrase("pass-iso-alpha")
        .with_mdns_query_interval(std::time::Duration::from_millis(100))
        .with_mdns_ttl(std::time::Duration::from_secs(5))
        .with_sync_interval(std::time::Duration::from_secs(2))
        .build()
        .await
        .expect("build node");
    node_alpha
        ...
        .sync()
        .await
        .expect("sync alpha schema");

    let node = node_alpha.node();
    let node_beta = node
        .join_group(&topic_beta, "pass-iso-beta", None)
        .await
        .expect("join beta group");
    node_beta
        .schema()
        .register(task::Entity)
        .sync()
        .await
        .expect("sync beta schema");

    // Two web-core peers, same table name "tasks" in both configs (see
    // `WebPeer::new`), isolated purely by which store/native-group pairing
    // each is exchanged against below.
    let mut web_a = WebPeer::new(75, DeletePolicy::DeleteWins);
    let mut web_b = WebPeer::new(76, DeletePolicy::DeleteWins);
    let web_b_baseline_digest = web_b.digest().await;
    assert!(
        web_b.state().await.is_empty(),
        "web store B starts with zero rows"
    );

    // Group A writes a row; exchange with web store A ONLY.
    task::ActiveModel {
        id: Set("a1".into()),
        title: Set("alpha one".into()),
        completed: Set(false),
    }
    .insert(&node_alpha)
    .await
    .unwrap();
    exchange_and_prove(&node_alpha, &mut web_a, "group A <-> web A").await;

    // web store B must be completely untouched by group A's exchange.
    assert!(
        web_b.state().await.is_empty(),
        "web store B must have zero rows after group A's exchange"
    );
    assert_eq!(
        web_b.digest().await,
        web_b_baseline_digest,
        "web store B's digest must be unchanged by group A's exchange"
    );

    let web_a_digest_after_a = web_a.digest().await;

    // Symmetric: group B writes a DIFFERENT row; exchange with web store B ONLY.
    task::ActiveModel {
        id: Set("b1".into()),
        title: Set("beta one".into()),
        completed: Set(false),
    }
    .insert(&node_beta)
    .await
    .unwrap();
    exchange_and_prove(&node_beta, &mut web_b, "group B <-> web B").await;

    // web store A must be completely unaffected by group B's exchange.
    assert_eq!(
        web_a.digest().await,
        web_a_digest_after_a,
        "web store A's digest must be unchanged by group B's exchange"
    );
    assert!(
        web_a.state().await.iter().all(|c| c.pk.0 != "b1"),
        "web store A must never see group B's row b1"
    );
    assert!(
        web_b.state().await.iter().all(|c| c.pk.0 != "a1"),
        "web store B must never see group A's row a1"
    );

    // The two native group DBs stay isolated from each other too (mirrors
    // `integration_multigroup.rs::test_multigroup_isolation`).
    assert!(
        task::Entity::find_by_id("a1")
            .one(&node_alpha)
            .await
            .unwrap()
            .is_some()
    ...
    assert!(
        task::Entity::find_by_id("b1")
            .one(&node_alpha)
            .await
            .unwrap()
            .is_none(),
        "group A's native DB must never see group B's row"
    );
    assert!(
        task::Entity::find_by_id("b1")
            .one(&node_beta)
    ...
    assert!(
        task::Entity::find_by_id("a1")
            ...
            .unwrap()
            .is_none(),
        "group B's native DB must never see group A's row"
    );
}
