...
//!
//! Node ID seeds: 80–89 (registered in CLAUDE.md §6); 74–76 for the
//! two-group isolation scenario (one native node's default-group identity,
//! plus two web-store site ids — the joined group's own site_id is always
//! freshly generated by `join_group`, never seed-controlled, so no seed is
//! needed for it). `make_node_id` takes a `u8`, so the brief's suggested
//! 260–263 doesn't fit the type; 74–76 is the nearest verified-free gap
//! (checked against every `make_node_id(..)` call across `tests/`, not just
//! the documented ranges — a couple of small values inside CLAUDE.md's
//! documented "1–10" range for `integration.rs` turned out to already be in
//! use undocumented, so grep ground-truth won out over the table here).
