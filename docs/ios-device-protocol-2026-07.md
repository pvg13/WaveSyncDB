...
3. Archive/run to BOTH physical devices (not Simulator — Simulator doesn't
   ...
   background-execution grants; see `CLAUDE.md`/research notes).
...
against the *shipped* configuration, independent of A's verdict.
...
For each pairing, after ~30s of connectivity (enough for a DCUtR attempt to
resolve one way or the other), read both diagnostics fields on EACH device.
...
3. Watch Device A's console for the `bg_sync` stage markers:
   ```
